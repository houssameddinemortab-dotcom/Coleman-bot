...
# ─────────────────────────────────────────────
#  BUDGET API 12DATA (plan gratuit = 800 crédits/jour)
#  Scan 5min = 2 appels × 288 = ~576/jour ✅
#  NE PAS descendre SCAN_INTERVAL_SEC en dessous de 300s
# ─────────────────────────────────────────────
SCAN_INTERVAL_SEC = 300   # 5 minutes
API_RETRY_MAX     = 3     # tentatives par source avant abandon
...
def get_candles(interval: str, outputsize: int = 100) -> list[dict]:
    """
    Récupère les bougies OHLCV XAU/USD depuis 12data.
    Retry automatique jusqu'à API_RETRY_MAX fois.
    Lève ValueError si quota journalier épuisé.
    """
    ...
    for tentative in range(1, API_RETRY_MAX + 1):
        try:
            ...
            if "values" not in data:
                msg = data.get("message", "Réponse invalide")
                raise ValueError(f"Erreur 12data : {msg}")
            candles = []
            ...
            last_error = e
            print(f"  ⚠ 12data tentative {tentative}/{API_RETRY_MAX} : {e}")
